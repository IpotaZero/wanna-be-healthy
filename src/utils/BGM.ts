class BGM {
    ...
    static async fetch({
        ...
        // 時間管理をリセット
        this.Time.isReversed = false
        this.Time.resetCurrentTime(this.context.currentTime)
        this.Time.setTime({
            loopStart: loopStart ?? 0,
            loopEnd: loopEnd ?? this.audioBuffer.duration,
    ...
    static play(): Promise<void> {
        ...
        if (this.checkFetched()) return Promise.reject()
    ...
    static reverse(): void {
        ...
        this.Time.isReversed = !this.Time.isReversed
        this.reconnect()
    }
    ...
    static setCurrentTime(second: number) {
        ...
        this.Time.setCurrentTime(this.context.currentTime, second)
        this.reconnect()
    }
    ...
    static getCurrentTime(): number {
        ...
        return this.Time.getCurrentTime(this.context.currentTime)
    }
    ...
    private static reconnect() {
        ...
        this.audio.buffer = this.Time.isReversed ? this.reversedBuffer : this.audioBuffer
        ;[this.audio.loopStart, this.audio.loopEnd] = this.Time.getActualLoopTime()
        ...
        const offset = this.Time.getActualOffset(this.context.currentTime)

        this.audio.start(0, offset)
    }

    private static checkFetched() {
        if (!this.audioBuffer) {
            console.error("BGM is not fetched!")
            return true
        }
...
namespace BGM {
    ...
    export class Time {
        ...
        private static lastUpdateContextTime = 0

        private static loopStart = 0
        private static loopEnd = 0
        ...
        static setTime({ loopStart, loopEnd, duration }: { loopStart: number; loopEnd: number; duration: number }) {
            this.loopStart = loopStart
            this.loopEnd = loopEnd
            this.duration = duration
        }

        static resetCurrentTime(contextTime: number) {
            this.currentTime = 0
            this.lastUpdateTime = 0
            this.lastUpdateContextTime = contextTime
        }

        static getCurrentTime(contextTime: number) {
            this.updateCurrentTime(contextTime)
        ...
        static getActualOffset(contextTime: number) {
            this.updateCurrentTime(contextTime)
        ...
        private static updateCurrentTime(contextTime: number) {
            // 必ず0以上
